  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(loanRequestsRef, (snapshot) => {
      ...
      if (data) {
        ...
        const activeLoanRequests = formattedData.filter(
          (request) =>
            request.status !== "accepted" && request.status !== "denied"
        );

        setLoanRequests(activeLoanRequests);
      } else {
        setLoanRequests([]);
      }
    });
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.main}>
        {loanRequests.length === 0 ? (
          <Text style={styles.noDataText}>No loan requests available.</Text>
        ) : (
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={styles.iconButton}
          onPress={() => router.push("/admin/accepted")}
        >
